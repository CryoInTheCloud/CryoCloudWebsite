...
def get_data_links(granules, environment="local"):
    """Returns links for in-cloud access by h5coro or for local access"""
    if environment == "local":
        return [g.data_links(access="out_of_region")[0] for g in granules]
    else:
        return [g.data_links(access="direct")[0].replace("s3://", "") for g in granules]
        
        
def get_credentials(environment="local"):
    """Returns access credentials for h5coro depending on environment"""
    if environment == "local":
...
def read_atl10(files, bounding_box=None, executors=4, environment="local", credentials=None):
    ...
    def read_h5coro(file):
        ...
        try:
            ...
            print(f"h5coro.readDatasets failed while reading ds_list for {file}")
            return
